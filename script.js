document.addEventListener("DOMContentLoaded", () => {
  ...
  function parseCalculatorLine(line){
    ...
    // Product line. Accept the site's standard format plus harmless wording/unit variations
    // so a valid KБЖВ line never silently turns into a zero-macro text entry.
    const re=/^(.+?),\s*для\s*([+-]?[\d.,]+)\s*(?:грам(?:ів|и|а)?|гр|г|мл)\s*-\s*([+-]?[\d.,]+)\s*(?:ккал|калор(?:ій|ії|ія|ійність)?)\s*\/\s*([+-]?[\d.,]+)\s*(?:г\s*)?(?:білка|білку|білків|білок)\s*\/\s*([+-]?[\d.,]+)\s*(?:г\s*)?(?:жирів|жиру|жири)\s*\/\s*([+-]?[\d.,]+)\s*(?:г\s*)?(?:вуглеводів|вуглеводи)(?:\s*\/\s*([+-]?[\d.,]+)\s*(?:г\s*)?(?:цукрів|цукру|цукри))?(?:\s*\/\s*([+-]?[\d.,]+)\s*(?:г\s*)?(?:солі|сіль))?(?:\s*\/\s*([+-]?[\d.,]+)\s*(?:г\s*)?(?:клітковини|клітковина))?\s*$/i;
    const m=clean.match(re);
    if(m)return{
      id:createId("calc"),
      name:m[1].trim(),
      weight:calculatorNumber(m[2]),
      kcal:calculatorNumber(m[3]),
      protein:calculatorNumber(m[4]),
      fat:calculatorNumber(m[5]),
      carb:calculatorNumber(m[6]),
      sugar:calculatorNumber(m[7]||0),
      salt:calculatorNumber(m[8]||0),
      fiber:calculatorNumber(m[9]||0),
      text:clean,
  ...
  function normalizeCalculatorItem(item){
    ...
    // If an old/stale calculator item contains a proper product summary in its text,
    // rebuild the macro values from that summary. This repairs previously saved entries
    // that displayed correctly but had zero/missing numeric fields.
    const reparsed=text?parseCalculatorLine(text):null;
    const looksLikeProduct=/\bккал\b/i.test(text)&&/\bжир/i.test(text)&&/\bвуглевод/i.test(text);
    if(looksLikeProduct&&reparsed){
      return {
        ...item,
        name:reparsed.name,
        weight:reparsed.weight,
        kcal:reparsed.kcal,
        protein:reparsed.protein,
        fat:reparsed.fat,
        carb:reparsed.carb,
        sugar:reparsed.sugar,
        salt:reparsed.salt,
        fiber:reparsed.fiber,
        text,
        id:item.id||reparsed.id,
        created_at:item.created_at||reparsed.created_at
      };
    }
  ...
  function updateTotals(){
    const t=calculatorItems.reduce((a,raw)=>{
